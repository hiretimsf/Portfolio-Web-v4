...
import { Toc } from "@/components/fuma/fuma-toc";

// Temporary stub for ActiveSectionObserver
...
// Wrapper for TOC to match expected usage if needed, or replace usage
const TableOfContents = () => (
  <Toc>
    {/* TOC items would be injected here if we were using fumadocs context correctly */}
    {/* For now, just render the container */}
  </Toc>
);

interface Props {
  post: BlogPostType & { headings?: Heading[] }; // Extend type to include headings if passed
}

const DetailBlogPost: FC<Props> = ({ post }) => {
  const { title, image, body: MDXBody, author, authorAvatar } = post;
  const MDXContent = MDXBody as React.ComponentType<any>;
  ...
  // If headings are missing from post, we might need another way to get them for TOC
  // But for now, we'll just handle the display errors.
              ...
              <div className="prose max-w-2xl overflow-hidden px-6 pt-4 pb-8 sm:px-8 sm:pt-6 sm:pb-12">
                <MDXContent
                  components={{
                    ...
                    h1: ({ children, id }: any) => (
                      <h1
                        id={id || slugify(children?.toString() || "")}
